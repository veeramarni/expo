...
async function setNotificationConfigAsync(props, manifest) {
  ...
  if (props.color) {
    addMetaDataItemToMainApplication(
      mainApplication,
      exports.META_DATA_NOTIFICATION_ICON_COLOR,
      exports.NOTIFICATION_ICON_COLOR_RESOURCE,
      'resource'
    );
  } else {
    removeMetaDataItemFromMainApplication(
...
async function setNotificationIconColorAsync(color, projectRoot) {
  ...
  if (color) {
    const colorItemToAdd = Resources_1.buildResourceItem({
...
async function writeNotificationIconImageFilesAsync(icon, projectRoot) {
  await Promise.all(
    Object.values(dpiValues).map(async ({ folderName, scale }) => {
      ...
      const dpiFolderPath = path_1.default.resolve(
        projectRoot,
        ANDROID_RES_PATH,
        drawableFolderName
      );
      ...
      try {
        const resizedIcon = (
          await image_utils_1.generateImageAsync(
        ...
        await fs_extra_1.default.writeFile(
          path_1.default.resolve(dpiFolderPath, exports.NOTIFICATION_ICON + '.png'),
          resizedIcon
        );
      } catch (e) {
...
async function setNotificationSoundsAsync(sounds, projectRoot) {
  ...
  await Promise.all(
    sounds.map(async soundFileRelativePath => {
...
async function writeNotificationSoundFileAsync(soundFileRelativePath, projectRoot) {
  const rawResourcesPath = path_1.default.resolve(projectRoot, ANDROID_RES_PATH, 'raw');
